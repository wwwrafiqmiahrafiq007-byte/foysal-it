...
export async function sendOutreach(messageId: string) {
  ...
  const [integration] = await db.select().from(leadPlatformIntegrations).where(and(eq(leadPlatformIntegrations.workspaceId, workspace.id), eq(leadPlatformIntegrations.providerKey, providerKey)));
  if (!integration || integration.status !== "Connected") {
    await db.update(outreachMessages).set({ providerStatus: "Integration Required", updatedAt: new Date() }).where(eq(outreachMessages.id, messageId));
    return { ok: false, status: 424, error: "Integration Required", message: `${message.channel === "whatsapp" ? "WhatsApp Business/API" : "Email Provider"} is not connected. Reconnect or configure integration settings before sending.`, retry: true, reconnect: true };
  }
  return { ok: false, status: 501, error: "Provider send adapter not implemented in this sandbox", message: "Credentials are detected but no tested provider adapter is enabled. This prevents fake send success responses." };
}
