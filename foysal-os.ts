...
export async function seedFoysalOsData() {
  ...
  await db
    ...
    .values({
      userId: owner.id,
      passwordHash: `scrypt-sha256$demo$${stableHash("demo-owner-password-not-exposed")}`,
...
export async function getFoysalOsSnapshot() {
  ...
  const currentEntitlements = subscription ? entitlementRows.filter((entitlement) => entitlement.planId === subscription.plan.id) : [];
